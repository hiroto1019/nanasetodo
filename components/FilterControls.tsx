...
const FilterControls: React.FC<FilterControlsProps> = ({ currentFilter, onFilterChange }) => {
  ...
  return (
    <div className="flex flex-wrap sm:flex-nowrap justify-center sm:justify-start space-x-2 sm:space-x-3 h-full">
      {filters.map((filter) => (
        <button
          key={filter.value}
          onClick={() => onFilterChange(filter.value)}
          className={`px-3 py-2.5 sm:px-4 text-sm font-medium rounded-md transition-colors duration-150 h-full
